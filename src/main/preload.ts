...
export interface ExposedApi {
  ...
  onShowTranslation: (callback: (result: TranslationEntry) => void) => void;
  onTranslationError: (callback: (originalText: string, error: string) => void) => void;
...
contextBridge.exposeInMainWorld("api", {
  ...
  loadTranslation: (entry: TranslationEntry) => ipcRenderer.invoke("load-translation", entry),
  onTranslationLoading: (callback: (originalText: string) => void) => {
    ipcRenderer.on("translation-loading", (_, originalText) => callback(originalText));
  },
  onShowTranslation: (callback: (result: TranslationEntry) => void) => {
    ipcRenderer.on("show-translation", (_, result) => callback(result));
  },
  onTranslationError: (callback: (originalText: string, error: string) => void) => {
    ipcRenderer.on("translation-error", (_, originalText, error) => callback(originalText, error));
  },
